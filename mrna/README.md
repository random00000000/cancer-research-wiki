# mRNA Cancer Knowledge Vanguard

Status: foundation pass
Last updated: 2026-06-14

Audience label: public education, patient question preparation, clinician discussion support, research hypothesis support, trial landscape review, not for clinical use.

This section is the mRNA cancer research spine for Cancer Research Wiki. It exists to make mRNA cancer vaccines and therapeutics understandable, evidence-grounded, and agent-retrievable without becoming a do-it-yourself treatment manual.

## Safety Boundary

mRNA cancer therapies are not recipes. A useful mRNA cancer product requires tumor tissue handling, sequencing, bioinformatics, antigen selection, mRNA design, delivery chemistry, sterile manufacturing, release testing, regulatory oversight, clinical monitoring, adverse-event management, and integration with standard oncology care.

Agents must not provide:

- DIY treatment design or manufacturing instructions.
- Dosing, self-administration, or procurement guidance.
- Instructions for making lipid nanoparticles, vaccines, biologics, or experimental drugs.
- Claims that an experimental mRNA product is a proven cure.

Safe purpose: help people understand the landscape, ask better questions, understand clinical trials, and collaborate with qualified clinicians and researchers.

## Foundation Pages

- [Plain-language mRNA cancer therapy explainer](public-mrna-cancer-therapy-explainer.md)
- [Tumor-to-vaccine clinical translation pathway](mrna-cancer-therapy-pathway.md)
- [Starter mRNA cancer trial and evidence landscape](trial-landscape.md)

## Mechanism Pages

- [Personalized mRNA neoantigen vaccines](personalized-neoantigen-vaccines.md)
- [Shared tumor-antigen mRNA vaccines](shared-tumor-antigen-vaccines.md)
- [mRNA delivery: lipid nanoparticles, lipoplexes, and targeting](delivery-lnp-and-lipoplex.md)
- [Antigen presentation and dendritic cells](antigen-presentation-and-dendritic-cells.md)
- [Immune escape, safety signals, and combination logic](immune-escape-safety-and-combinations.md)

## Source Cards

- [KEYNOTE-942 / mRNA-4157-V940 in resected melanoma](../sources/mrna/keynote-942-mrna-4157-v940-melanoma.md)
- [Autogene cevumeran personalized mRNA-lipoplex trials](../sources/mrna/autogene-cevumeran-solid-tumors-and-pdac.md)
- [BNT111 FixVac shared-antigen mRNA vaccine in melanoma](../sources/mrna/bnt111-fixvac-shared-antigen-melanoma.md)
- [LNP and mRNA cancer vaccine delivery reviews](../sources/mrna/lnp-and-mrna-cancer-vaccine-delivery-reviews.md)

## First Public Visual

- `visuals/mrna-cancer-therapy-pathway.svg`
- First localized variants: `visuals/mrna-cancer-therapy-pathway.es.svg`, `visuals/mrna-cancer-therapy-pathway.de.svg`, `visuals/mrna-cancer-therapy-pathway.zh.svg`, and `visuals/mrna-cancer-therapy-pathway.ru.svg`

## Current Vanguard Reading

The strongest current signal is not "mRNA cures cancer." The stronger and safer reading is:

- Personalized mRNA neoantigen vaccines are becoming clinically credible in selected adjuvant settings, especially high-risk resected melanoma when combined with checkpoint blockade.
- Pancreatic ductal adenocarcinoma has important early evidence that a personalized mRNA-lipoplex vaccine can generate durable neoantigen-specific T cells in some patients, but the clinical dataset is small and phase 2 testing is ongoing.
- Delivery, antigen choice, immune context, tumor burden, HLA presentation, combination therapy, manufacturing time, and resistance mechanisms can decide whether the idea works.
- Many mRNA cancer claims remain early, preclinical, company-reported, or trial-in-progress.

## Source Mix For This First Pass

- Randomized phase 2b melanoma trial: https://pubmed.ncbi.nlm.nih.gov/38246194/
- Trial record for mRNA-4157/V940 KEYNOTE-942: https://clinicaltrials.gov/study/NCT03897881
- Phase 3 adjuvant melanoma trial: https://clinicaltrials.gov/study/NCT05933577
- Company-reported five-year melanoma follow-up, to treat cautiously until peer-reviewed: https://www.merck.com/news/moderna-merck-announce-5-year-data-for-intismeran-autogene-in-combination-with-keytruda-pembrolizumab-demonstrated-sustained-improvement-in-the-primary-endpoint-of-recurrence-free-survival-i/
- Pancreatic cancer mRNA-lipoplex neoantigen vaccine follow-up: https://www.nature.com/articles/s41586-024-08508-4
- Pancreatic phase 2 trial: https://clinicaltrials.gov/study/NCT05968326
- LNP-based mRNA cancer vaccine review: https://pmc.ncbi.nlm.nih.gov/articles/PMC11336688/
- BNT111 shared-antigen melanoma FixVac source card anchors: https://pubmed.ncbi.nlm.nih.gov/32728218/ and https://investors.biontech.de/news-releases/news-release-details/biontech-announces-positive-topline-phase-2-results-mrna/

## Next Build Targets

- Expand source cards for mRNA-4157/V940, autogene cevumeran, LNP delivery, mRNA-lipoplex delivery, and shared-antigen mRNA vaccines with more full-text details and contradiction sections.
- Expand mechanism pages for HLA restriction, innate immune sensing, manufacturing constraints, safety signals, and checkpoint-combination logic.
- Cancer-type pages for melanoma, pancreatic ductal adenocarcinoma, non-small cell lung cancer, bladder cancer, renal cell carcinoma, colorectal cancer, and glioblastoma.
- Trial table with NCT identifiers, platform, cancer type, phase, endpoint, toxicity, and status.
- Visuals for LNP delivery, neoantigen selection, immune response, manufacturing constraints, and trial endpoints.
